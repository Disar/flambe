...
class KhaAudioSound extends BasicAsset<KhaAudioSound>
    implements Sound
{
    ...
    override private function copyFrom (that :KhaAudioSound)
    {
    ...
    implements Tickable
{
    ...
    public function new (sound :KhaAudioSound, volume :Float, loop :Bool)
    {
        _sound = sound;
        _head = KhaAudioSound.gain;
        _complete = new Value<Bool>(false);

        _sourceNode = KhaAudioSound.ctx.createBufferSource();
        _sourceNode.buffer = sound.buffer;
        _sourceNode.loop = loop;
        _sourceNode.onended = function () _complete._ = true; // Not supported on iOS!
        KhaAudioSound.start(_sourceNode, 0);
        playAudio();
    ...
    public function get_position () :Float
    {
        ...
        if (_complete._) {
            ...
            var elapsed = KhaAudioSound.ctx.currentTime - _startedAt;
    ...
    {
        if (_gainNode == null) {
            _gainNode = KhaAudioSound.createGain();
    ...
    private function playAudio ()
    {
        ...
        _startedAt = KhaAudioSound.ctx.currentTime;
        _pausedAt = -1;
    ...
    private var _sound :KhaAudioSound;
